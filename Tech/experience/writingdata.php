...
$sql = "SELECT * FROM `experience` ORDER BY tag DESC ";

$result = mysqli_query($connect,$sql);
$num_rows = mysqli_num_rows($result);
if($_SESSION['id'] != null ) {
    $id = $_SESSION['id'];
    echo "<a href='experience/writingRe.php'><button class=\"button button1\" >新增</button></a> <br>";
}
echo " 共有 $num_rows 項 \n";
...
<!--        <p> --><?php //echo " $row[2]" ?><!--</p>-->
            ...
            ?>
                ...
                <form name="form" method="post" action="experience/writing_update.php">
                    <?php
                    echo"<input  type=\"hidden\" name=\"tag\" value=\"$row[0]\" />";
                    ?>
                    <input type="submit" class="button button2" name="button" value="修改" />
                </form>
                <form name="form" method="post" action="experience/writing_delete_finish.php">
                    <?php
                    echo"<input  type=\"hidden\" name=\"tag\" value=\"$row[0]\" />";
                    ?>
