...
$sql = "SELECT * FROM `writingdc` ORDER BY tag DESC ";

$result = mysqli_query($connect,$sql);
$num_rows = mysqli_num_rows($result);
if($_SESSION['id'] != null ) {
    $id = $_SESSION['id'];
    echo "<a href='dc/writingRe.php'><button class=\"button button1\" >新增</button></a> <br>";
}
echo " 共有 $num_rows 篇 \n";
        ...
        <div class="well">
           ...
           ?>
            ...
            ?>
                ...
                <form name="form" method="post" action="dc/writing_update.php">
                    <?php
                    echo"<input  type=\"hidden\" name=\"tag\" value=\"$row[0]\" />";
                    ?>
                    <input type="submit" class="button button2" name="button" value="修改" />
                </form>
                <form name="form" method="post" action="dc/writing_delete_finish.php">
                    <?php
                    echo"<input  type=\"hidden\" name=\"tag\" value=\"$row[0]\" />";
                    ?>
